...
void CBar::ElementGravity(double* bodyforce, double Gravity)
{
	//	Calculate bar length
	double DX[3];		//	dx = x2-x1, dy = y2-y1, dz = z2-z1
	for (unsigned int i = 0; i < 3; i++)
		DX[i] = nodes_[1]->XYZ[i] - nodes_[1]->XYZ[i];
...
//	Calculate element stiffness matrix
	...
	CBarMaterial* material = dynamic_cast<CBarMaterial*>(ElementMaterial_);	// Pointer to material of the element
	double F0 = 0.5 * material->rho * Gravity * L * material->Area;
	bodyforce[0] = 0;
	bodyforce[1] = F0;
	bodyforce[2] = 0;
	bodyforce[3] = 0;
...
//	Assemble the element body force to the global body force
void CBar::assemblygravity(double* bodyforce, double* Force, double Gravity)
{
//	Calculate element bodyforce
	ElementGravity(bodyforce, Gravity);
	
//	Assemble global bodyforce
	for (unsigned int j = 0; j < ND_; j++)
	{
		unsigned int Lj = LocationMatrix_[j];	// Global equation number corresponding to jth DOF of the element
        Force[Lj] += bodyforce[j];
	}
